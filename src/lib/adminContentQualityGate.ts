...
const validatePublishedJobQuality = (entry: AdminMobileJobEntry) => {
  ...
  if (!sourceUrl || !["http:", "https:"].includes(sourceUrl.protocol)) {
    issues.push("Add the official source URL used to verify this job. Do not publish from an unverified repost.");
  }

  if (!entry.sourceCheckedAt) {
    issues.push("Add the date this official source was checked.");
  }
...
export const validatePublishedContentQuality = (entry: AdminMobileEntry) => {
  ...
  const sourceUrl = safeParseUrl(entry.sourceUrl);
  if (!sourceUrl || !["http:", "https:"].includes(sourceUrl.protocol)) {
    issues.push("Published jobs require an official source URL.");
  }
  if (!entry.sourceCheckedAt) {
    issues.push("Published jobs require a source-checked date.");
  }
  return issues;
};
